
var resourceName = 'BaseProperty'//资源名称 模板生成
var apiName = JSON.parse(window.coolLocals['index.json'])['apiName']
window.vm = new Vue({
  el: '#root',
  data: {
    // uniqueDeployKeyURL为前端已定义的变量 面的EmployeeInfo为后端模板生成的文件变量名 后面的为固定格式 需模板生成
     uniqueDeployKey:{
      api: apiDict[apiName] + resourceName
     },
     // axiosSetting 固定格式 需模板生成生成
     axiosSetting:{
        baseURL:apiDict[apiName],
      },
      // cool-single-dialog组件的json文件名以及它的api名称 uniqueKeyURL为前端已定义的变量 后面的EmployeeInfo为后端模板生成的文件变量名
     isMethods:{
      isGetCondition:false,
      isTableSelectionChange:false,
      isQuerySearch:false
     },
     showModeList:true,

      // 弹出框 固定格式 里面的值可按以下定义 需模板生成
     dialogs: [{
          top: '5vh',
          name: 'dialog',
          visible: false,
          collapse: false,
          width: '90%',
          title: '',
          src: '',
        }
      ],
     //cool-single-dialog 
     uniqueKey: apiDict[apiName] + resourceName,
     // 是否显示cool-single-dialog组件 默认值固定为false 需模板生成
     dialogVisible:false,
     isDialogMethods:{
      isUpdateForm:false,
      isSaveEvent:false
     },
     watchData:[]
     // activeNameValue:undefined
  },
  mounted() {
    // 固定格式 需模板生成
    this.$el.style.visibility = 'visible'
    
   this.watchData = this.$refs.singleView.singleTableData
   
  },
  watch:{
    'watchData.data':function(arg){
      console.log(arg)
      // 每次数据新增之后 更新请求数据 以便同步 查询 的类型下拉数据
      this.getTypeData()
    }
  },
  methods: {
    // 类型数据获取
    getTypeData(){
      axiosDict[apiName].get('BaseProperty/GetTypes').then(res=>{
      this.$refs.singleView.queryCondition.Type.options = []
       let newData = []
      res.map(item=>{
            this.$refs.singleView.queryCondition.Type.options.push({value:item})                               
          })
      
    })
    },
    // 弹窗slot插入数据的一种方法
    // gaveData(){
    //   let ruleForm = this.$refs.singleDialog.$refs.ruleForm
    //   ruleForm.formItems.form.map(item=>{
    //     if(item.name == 'activeName')item.value = this.activeNameValue
    //   })
    //   ruleForm.updateForm()
    // },
      //cool-single-view 
      tableRowClick(arg){
      
      },
      tableRowDblclick(arg){
    
      },
      tableSelectionChange(arg){
    
      },  
      paginationSizeChange(arg){

      },
      paginationCurrentChange(arg){

      },
      getCondition(arg){

      },
      // cool-single-dialog
      updateForm(arg){

      },
      saveEvent(arg){

      }
  }
})
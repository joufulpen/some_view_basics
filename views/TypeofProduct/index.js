var resourceName = 'TypeofProduct'//资源名称 模板生成
...
window.vm = new Vue({
  ...
  methods: {
    getOptionsData(){
      axiosDict[apiName].get(this.uniqueKey + '/GetList?condition=[]').then(res=>{
        console.log(res)
        if(res){
      ...
      customNew(){
        ...
        axiosDict[apiName].get(this.uniqueKey + '/newItem').then(res=>{
          console.log(res)
